...
namespace BatteryNotifier.Forms {
	...
	public partial class Dashboard : Form {
		...
		private BatteryNotification[] HighBatteryNotifications;
		private BatteryNotification[] LowBatteryNotifications;
		...
		private void LoadSettings() {
			...
			BatteryNotificationSettingsDto[] lowNotificationSettings;
			{
				var settings = appSetting.Default;
				lowNotificationSettings = new BatteryNotificationSettingsDto[] {
						new BatteryNotificationSettingsDto("Low Battery", settings.lowBatteryNotificationMessageTemplate, settings.lowBatteryNotificationValue, settings.lowBatteryNotificationMusic, settings.lowBatteryNotification, settings.lowBatteryNotificationDuration, maxOncePerThreshold:true),
						new BatteryNotificationSettingsDto("Low Battery", settings.lowBatteryNotificationMessageTemplate2, settings.lowBatteryNotificationValue2, settings.lowBatteryNotificationMusic, settings.lowBatteryNotification2, settings.lowBatteryNotificationDuration2, maxOncePerThreshold:true),
						new BatteryNotificationSettingsDto("Low Battery", settings.lowBatteryNotificationMessageTemplate3, settings.lowBatteryNotificationValue3, settings.lowBatteryNotificationMusic, settings.lowBatteryNotification3, settings.lowBatteryNotificationDuration3, maxOncePerThreshold:true)
					};
			}

			if (LowBatteryNotifications is null) {
				LowBatteryNotifications = new BatteryNotification[3];
				for (int i = 0; i < LowBatteryNotifications.Length; i++) {
					new BatteryNotification(lowNotificationSettings[i], canNotify: true);
				}
			}
			else {
				for (int i = 0; i < LowBatteryNotifications.Length; i++)
					LowBatteryNotifications[i].Settings = lowNotificationSettings[i];
			}

			BatteryNotificationSettingsDto[] highNotificationSettings;
			{
				var settings = appSetting.Default;
				highNotificationSettings = new BatteryNotificationSettingsDto[] {
					new BatteryNotificationSettingsDto(settings.fullBatteryNotificationTitle, settings.fullBatteryNotificationMessageTemplate, settings.fullBatteryNotificationValue, settings.fullBatteryNotificationMusic, settings.fullBatteryNotification, TimeSpan.FromSeconds(50), maxOncePerThreshold:true)
				};
			}

			if (HighBatteryNotifications is null) {
				HighBatteryNotifications = new BatteryNotification[3];
				for (int i = 0; i < HighBatteryNotifications.Length; i++) {
					new BatteryNotification(highNotificationSettings[i], canNotify: true);
				}
			}
			else {
				for (int i = 0; i < HighBatteryNotifications.Length; i++)
					HighBatteryNotifications[i].Settings = highNotificationSettings[i];
			}
		...
		private void CheckNotification() {
			if (powerStatus.PowerLineStatus == PowerLineStatus.Online && _isCharging) {
				foreach (var notif in this.HighBatteryNotifications.OrderByDescending(n => n.Settings.ThresholdReal)) {
					var notifSettings = notif.Settings;

					if (powerStatus.BatteryLifePercent >= notifSettings.ThresholdReal) {
						string message = notif.GetMessage();
						string title = notif.Title;
						Notify(message);
						if (notifSettings.ShowNotification) {
							BatteryNotifierIcon.ShowBalloonTip((int)notifSettings.Duration.TotalSeconds, title, message, ToolTipIcon.Info);
							PlaySound(appSetting.Default.fullBatteryNotificationMusic, Resources.BatteryFull, true);
						}
						notif.CanNotify = false;
						break;
					}
				}
			}
			...
			//reset 'notify once' flags
			foreach (var notif in this.HighBatteryNotifications) {
				var notifSettings = notif.Settings;
				notif.CanNotify = powerStatus.BatteryLifePercent < notifSettings.ThresholdReal || !notifSettings.MaxOncePerThreshold;
			}
			...
			if (powerStatus.PowerLineStatus == PowerLineStatus.Offline
				...
				foreach (var notif in this.LowBatteryNotifications.OrderBy(n => n.Settings.ThresholdReal)) {
					var notifSettings = notif.Settings;
			...
			//reset 'notify once' flags
			foreach (var notif in this.LowBatteryNotifications) {
				var notifSettings = notif.Settings;
				notif.CanNotify = powerStatus.BatteryLifePercent > notifSettings.ThresholdReal || !notifSettings.MaxOncePerThreshold;
			}
		}
